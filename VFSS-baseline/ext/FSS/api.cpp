                  ...
                  int32_t scale, std::string prefix) 
{
    ...
    if (party != DEALER) {
        ...
        FSS::stat_t stat = {prefix + "BPGCNSoftmax_Total", 0, static_cast<uint64_t>(time_taken), 0, 0, 0};
        FSS::push_stats(stat);
    }
...
struct CavernOnlineState
{
    CavernAuthShare x;

    CavernShiftedSpline shifted;
    CavernQueryPlan plan;

    VIDPFEvalResult mainEval;
    VIDPFEvalResult tagEval;

    std::vector<CavernAuthShare>
        selectedCoeff;

    CavernAuthShare polyValue;
};

static bool cavernBlockEqual(
...
static osuCrypto::block cavernCommitHash(
    const osuCrypto::block &message,
    const osuCrypto::block &nonce,
    uint64_t domain)
{
    ...
    static const block key =
        toBlock(
            0x43415645524E434FULL,
            0x4D4D49544D454E54ULL);
...
static osuCrypto::block cavernWideToBlock(
    const WideGroupElement &value)
{
    return osuCrypto::toBlock(
        value.hi,
        value.lo);
}

static GroupElement cavernCoinTossGE(
    int bw,
    uint64_t domain)
{
    using namespace osuCrypto;
    ...
    const block localSeed =
        FSSConfig::prngs[tid]
            .get<block>();

    const block localNonce =
        FSSConfig::prngs[tid]
            .get<block>();

    const block localCommitment =
        cavernCommitHash(
            localSeed,
            localNonce,
            domain);
    ...
    const block remoteSeed =
        peer->recv_block();

    const block remoteNonce =
        peer->recv_block();

    numRounds += 1;

    const block expectedCommitment =
        cavernCommitHash(
            remoteSeed,
            remoteNonce,
            domain);
    ...
    {
        std::cerr
            << "CAVERN coin-toss commitment check failed"
            << std::endl;

        std::abort();
    }

    static const block deriveKey =
        toBlock(
            0x43415645524E434FULL,
            0x494E544F53534545ULL);

    AES derive(deriveKey);

    const block combinedSeed =
        localSeed ^
        remoteSeed ^
        toBlock(domain, 0xC0);

    const block challengeBlock =
        derive.ecbEncBlock(
            combinedSeed)
        ^
        combinedSeed;

    GroupElement challenge =
...
static WideGroupElement cavernCommitRevealWide(
    const WideGroupElement &localValue,
    uint64_t domain)
{
    using namespace osuCrypto;
    ...
    const WideGroupElement remoteValue =
        peer->recv_wide_ge();
    ...
    {
        std::cerr
            << "CAVERN commit/open check failed"
            << std::endl;
...
static std::vector<GroupElement> cavernOpenBatch(
    int bin,
    int ringBw,
    const std::vector<CavernAuthShare> &localShares,
    const std::vector<CavernAuthShare> &upperMasks,
    std::vector<CavernOpenedValue> &openedRecords)
{
    always_assert(
        localShares.size() ==
        upperMasks.size());
         ...
         ++i)
    {
        ...
        sendValues[i] =
            maskedShares[i].value;
    }

    peer->send_wide_ge_array(
        sendValues.data(),
        count);

    peer->recv_wide_ge_array(
        receiveValues.data(),
        count);
    ...
    for (int i = 0;
         ...
         ++i)
    {
        const WideGroupElement openedFull =
            wideAdd(
                sendValues[i],
                receiveValues[i],
                ringBw);
        ...
        CavernOpenedValue record;

        record.localMaskedShare =
            maskedShares[i];

        record.openedFull =
            openedFull;

        openedRecords.push_back(
            record);
    }
...
static void cavernBatchMACCheck(
    int ringBw,
    const WideGroupElement &deltaShare,
    const std::vector<CavernOpenedValue> &records)
{
    if (records.empty())
    {
        return;
    ...
    const GroupElement challengeGE =
        cavernCoinTossGE(
            std::min(64, ringBw),
            0x42415443484D4143ULL);
    ...
    WideGroupElement localMacSum(0);
    WideGroupElement publicValueSum(0);
    ...
    const WideGroupElement localCheck =
        wideSub(
            localMacSum,
    ...
    const WideGroupElement remoteCheck =
        cavernCommitRevealWide(
            localCheck,
            0x424154434856414CULL);
...
static void cavernExchangeAndCheckVIDPF(
    int ringBw,
    const std::vector<CavernOnlineState> &states,
    const std::vector<WideGroupElement> &omega)
{
    const int size =
        static_cast<int>(
            states.size());

    std::vector<VIDPFToken>
        remoteMain(size);
    ...
    for (int i = 0;
         ...
         ++i)
    {
        peer->send_vidpf_token(
            states[i].mainEval.mu);
         ...
         ++i)
    {
        ...
        {
            std::cerr
                << "CAVERN consistency check failed at element "
                << i
                << std::endl;
    ...
    std::string prefix)
{
    ...
    always_assert(ringBw <= 128);

    for (const auto &polynomial : coeffs)
    {
        always_assert(
            static_cast<int>(
                polynomial.size())
    ...
    // ========================================================================
    // Dealer: preprocessing and key distribution.
    // ========================================================================
    ...
    if (party == DEALER)
    {
        ...
        // The same global MAC-key shares are reused inside this batch.
...
#pragma omp parallel for
        ...
        for (int i = 0;
             ...
             ++i)
        {
            server->send_cavern_spline_key(
                keys[i].first);

            client->send_cavern_spline_key(
                keys[i].second);

            freeCavernSplineKeyPackPair(
                keys[i]);
        }
    ...
    // ========================================================================
    // Server / Client: read preprocessing keys.
    // ========================================================================

    std::vector<CavernSplineKeyPack>
        keys(size);

    const uint64_t keysizeStart =
        dealer->bytesReceived();

    const uint64_t keyreadTime =
        time_this_block(
            [&]()
            {
                for (int i = 0;
    ...
    std::vector<CavernOpenedValue>
        openedRecords;

    openedRecords.reserve(
        size *
        (2 * degree + 2));

    const uint64_t onlineCommStart =
        peer->bytesSent() +
        peer->bytesReceived();
    ...
    std::vector<CavernOnlineState>
        states(size);

    // ========================================================================
    // Step 1: authenticate x and open x-alpha.
    // ========================================================================
         ...
         ++i)
    {
        ...
        xAlphaOpeningMasks[i] =
            keys[i].openMasks[0];
    }

    const std::vector<GroupElement> shifts =
        cavernOpenBatch(
            bin,
            ringBw,
            xMinusAlpha,
    ...
    // ========================================================================
    // Step 2: shift spline and evaluate the two VIDPFs.
    // ========================================================================
         ...
         ++i)
    {
        ...
        states[i].plan =
            cavernBuildQueryPlan(
                bin,
                states[i].shifted.endpoints);

        states[i].mainEval =
            evalVIDPF(
                partyId,
                keys[i].mainVIDPF,
                states[i].plan.queries);

        const std::vector<VIDPFQuery>
            tagQueries =
                cavernBuildTagQueries(
                    bin,
                    keys[i].tagA,
                    keys[i].tagB,
                    states[i].plan.queries);
        ...
        const std::vector<CavernAuthShare>
            indicators =
                cavernBuildIndicators(
                    partyId,
                    ringBw,
                    states[i].plan,
                    states[i].mainEval,
        ...
        states[i].selectedCoeff =
            cavernSelectPolynomial(
                ringBw,
                degree,
                indicators,
                states[i].shifted.coeffs);

        states[i].polyValue =
            states[i].selectedCoeff[degree];
    }

    // ========================================================================
    // Step 3: authenticated Horner evaluation.
    // ========================================================================
         ...
         ++round)
    {
        ...
        const std::vector<GroupElement> opened =
            cavernOpenBatch(
                bin,
                ringBw,
                valuesToOpen,
             ...
             ++i)
        {
            ...
            CavernAuthShare product =
                cavernFinishAuthMultiply(
                    partyId,
                    ringBw,
                    keys[i].polyTriples[round],
                    e,
    ...
    // ========================================================================
    // Step 4: aggregate and verify the two VIDPF evaluations.
    // ========================================================================

    const GroupElement batchingChallenge =
        cavernCoinTossGE(
            bin,
            0x56494450464F4D45ULL);
    ...
    cavernExchangeAndCheckVIDPF(
        ringBw,
        states,
        omega);

    // ========================================================================
    // Step 5: open the masked output.
    // ========================================================================
         ...
         ++i)
    {
        maskedOutput[i] =
            cavernAuthAdd(
                states[i].polyValue,
                keys[i].outputMaskShare,
                ringBw);

        outputOpeningMasks[i] =
            keys[i].openMasks[
                1 + 2 * degree];
    }

    const std::vector<GroupElement> openedOutput =
        cavernOpenBatch(
            bin,
            ringBw,
            maskedOutput,
    ...
    // ========================================================================
    // Step 6: check the MAC of every value opened during this invocation.
    // ========================================================================
