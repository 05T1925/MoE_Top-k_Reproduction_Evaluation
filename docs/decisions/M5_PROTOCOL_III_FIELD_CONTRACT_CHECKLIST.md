# M5 Protocol III Field Contract 评审检查表

状态：**设计评审检查表；不构成实现、测试或复现证据**

本文用于评审：

```text
docs/decisions/M5_PROTOCOL_III_PAPER_EVIDENCE.md
docs/decisions/M5_PROTOCOL_III_FIELD_CONTRACT.md
```

实际测试环境、命令、revision 和运行结果将在 field adapter 实现后写入独立的：

```text
docs/reproduction/M5_FIELD_ADAPTER_<ENVIRONMENT>_<DATE>.md
```
...
- [ ] 文档已记录 Rabin 不可约性判据及核验结果。
- [ ] 后续 field conformance test 已要求复算该判据。
