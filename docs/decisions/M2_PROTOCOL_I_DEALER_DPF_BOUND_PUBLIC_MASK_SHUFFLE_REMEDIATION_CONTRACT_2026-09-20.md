...
The normative byte-level entropy derivation contract is
[M2CBKDF1](M2_PROTOCOL_I_DEALER_DPF_ENTROPY_DERIVATION_CONTRACT_2026-09-21.md).
That `M2CBKDF1` decision is the sole normative source for entropy acquisition and HKDF byte semantics, identifier derivation, domain encoding, FSS `SetSeed` mapping, and the production versus TEST_ONLY API boundary. This record preserves all other remediation semantics.
...
Current fact: keyGenDPF/keyGenDCF use `FSSConfig::prngs[omp_get_thread_num()]` and accept no explicit PRNG. Candidate B requires a dedicated P2, serialized outside-OpenMP FSS lane, thread 0, and a process lock. Each DPF/DCF seed derivation and FSS `SetSeed` mapping is governed exclusively by the normative M2CBKDF1 contract above; the lock spans SetSeed and the complete key-generation call, with no concurrent global PRNG user or fixed seed. If impossible, the only fallback is a minimal explicit-PRNG overload; do not redesign FSS mathematics.
...
The sole production operation is conceptually `generate_candidate_b_material(validated_public_config)`: atomically bind `src -> DPF selectors -> u -> pi(u) -> independent u/v shares` and `r_A -> independent r_A shares -> R2 masks -> ordered uCMP/DCF material`. The production API accepts no entropy provider, root, PRK, raw seed, or caller-supplied identifier/fingerprint; no API independently replaces selector/payload/field selector/v/r_A/GRank material. IDs are defense-in-depth only; binding is private atomic construction.
