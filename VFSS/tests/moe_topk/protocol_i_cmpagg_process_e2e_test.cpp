// TEST_ONLY: forked local IPC validates bounded framing and counter boundaries.
#include <moe_topk/protocol_i_transport.h>
#include <sys/socket.h>
#include <sys/wait.h>
#include <unistd.h>
#include <stdexcept>
using namespace moe_topk; namespace {void ok(bool x){if(!x)throw std::runtime_error("process E2E");} ProtocolIFrameConfig c(int s,int r){return {7,9,5,3,36,(std::uint8_t)s,(std::uint8_t)r,1,1};}}
