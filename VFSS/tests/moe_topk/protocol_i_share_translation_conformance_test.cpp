...
int main(int ac,char** av){try{if(ac>1)return role_main(ac,av);::signal(SIGPIPE,SIG_IGN);invalid();const auto e=self(av[0]);std::uint64_t m=100;for(const auto t:{2U,4U,8U,16U,32U})for(int mode=0;mode!=5;++mode)run(e,cfg(t,m++),permutation(t,mode));for(int trial=0;trial!=3;++trial)run(e,cfg(16,m++),permutation(16,4+trial));return 0;}catch(const std::exception& x){std::cerr<<x.what()<<'\n';return 1;}}
