...
void wait_fail(pid_t p) { int s{}; require(::waitpid(p,&s,0)==p&&WIFEXITED(s)&&WEXITSTATUS(s)!=0,"role unexpectedly succeeded"); }
...
void expect_mismatch(const std::string& exe, ProtocolIOpvConfig fvo, ProtocolIOpvConfig po, bool fvo_on_both) { int ot[2]{},fr[2]{},pr[2]{},pin[2]{};pair(ot);pair(fr);pair(pr);pair(pin);const auto f=spawn(exe,role_args("fvo",ot[0],fr[1],-1,fvo));const auto p=spawn(exe,role_args(fvo_on_both?"fvo":"po",ot[1],pr[1],fvo_on_both?-1:pin[1],po));close_fd(ot[0]);close_fd(ot[1]);close_fd(fr[0]);close_fd(fr[1]);close_fd(pr[0]);close_fd(pr[1]);if(!fvo_on_both){std::vector<std::uint32_t> q(po.batch_count);write_all(pin[0],q.data(),q.size()*sizeof(std::uint32_t));}close_fd(pin[0]);close_fd(pin[1]);wait_fail(f);wait_fail(p);}
void negative_matrix(const std::string& exe) { int x[2]{};pair(x); bool threw=false;for(const ProtocolIOpvConfig c:std::vector<ProtocolIOpvConfig>{{1,2,3,0,1,1},{1,2,4,2,0,1},{1,2,5,32,UINT32_MAX,1}}){try{(void)moe_topk::protocol_i_opv_full_vector_owner(c,x[0]);}catch(...){threw=true;}require(threw,"invalid OPV shape");threw=false;}try{(void)moe_topk::protocol_i_opv_puncture_owner({1,2,6,2,1,1},x[0],{2});}catch(...){threw=true;}require(threw,"puncture range");close_fd(x[0]);close_fd(x[1]);const auto c=config(2,1,0x9000);auto other=c;++other.session;expect_mismatch(exe,c,other,false);other=c;++other.fingerprint;expect_mismatch(exe,c,other,false);other=c;++other.material_id;expect_mismatch(exe,c,other,false);expect_mismatch(exe,c,c,true);}
}  // namespace
int main(int argc,char** argv){try{if(argc>1)return role_main(argc,argv);::signal(SIGPIPE,SIG_IGN);invalid_inputs();const auto exe=self(argv[0]);negative_matrix(exe);std::uint64_t material=1;for(const auto t:{2U,4U,8U,16U,32U})for(const auto b:{1U,3U,t}){std::vector<std::uint32_t> p(b);for(std::uint32_t i=0;i<b;++i)p[i]=(i*3+b-1)%t;run_case(exe,config(t,b,material++),p);}return 0;}catch(const std::exception& e){std::cerr<<e.what()<<'\n';return 1;}}
